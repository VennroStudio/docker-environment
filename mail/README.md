...
|-----|-----|-----------------------------------------------------------|
| A | mail | `<IP_сервера>`                                            |
| MX | @ | `mail.domain.ru` (приоритет 10)                           |
| TXT | @ | `v=spf1 mx ~all`                                          |
| TXT | _dmarc | `v=DMARC1; p=quarantine; rua=mailto:postmaster@domain.ru` |
| TXT | dkim._domainkey | *(добавляется после генерации ключа)*                     |

## Получение DKIM-ключа

1. Зайди в админку: **Domains → vennro.ru → DKIM key**
2. Скопируй значение вида: ***v=DKIM1; k=rsa; p=ДЛИННЫЙ_КЛЮЧ***
3. Добавь в DNS:
    - **Тип:** TXT
    - **Имя:** `dkim._domainkey`
    - **Значение:** *(скопированное значение)*

## Mailu

Далее нужно заполнить `.env`. Сейчас от `.env.example` отличаются эти поля, их надо проверить на каждом сервере:
