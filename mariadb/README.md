...
Папка `dumps/` — место для бэкапов/дампов базы (например, результат `mysqldump`). В `.gitignore` стоит добавить `dumps/*.sql` (или всю папку), чтобы дампы с реальными данными не попадали в git.
